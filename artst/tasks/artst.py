...
class ArTSTTask(LegacyFairseqTask):
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        # Thesis: defining dataloader for multitask
            ...
            logger.info(
                "Task: {0}, Loaded {1} samples of speech-to-text_dataset".format(
                    'multitask',
    ...
    def train_step(
        ...
        def forward_backward(model, samples, weight=1.0):
            ...
            # asr_samples += len(samples) if logging_output['task'] == 's2t' else 0
            # tts_samples += len(samples) if logging_output['task'] == 't2s' else 0
            
            # weight = 1.5 if logging_output['task'] == 's2t' else 1.0
            if ignore_grad:
                loss *= 0
            else:
                loss *= weight
