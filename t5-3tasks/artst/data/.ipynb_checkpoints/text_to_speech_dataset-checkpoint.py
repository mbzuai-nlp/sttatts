...
def create_speaker_embedding(waveform):
    spk_model_name = "/root/TTS/v1.2/SpeakerEncoder/"
    ...
    with torch.no_grad():
        ...
        speaker_embeddings = speaker_model.encode_batch(waveform, normalize=False)
...
class TextToSpeechDataset(FairseqDataset):
    ...
    def __getitem__(self, index):
        if not self.inference:
            ...
            import soundfile as sf
            wav_path = '/root/TTS/v1.2/data/ASC/ASC_test/ARA NORM  0001.wav'
            wav, cur_sample_rate = sf.read(wav_path)
            wav = torch.from_numpy(wav).float()
            fbank = logmelfilterbank(
                wav.view(-1).cpu().numpy(), 16000
        ...
        if os.path.exists(os.path.join(self.audio_root, self.spk_embeds[index])):
            # print(f"From path: {os.path.join(self.audio_root, self.spk_embeds[index])} for audio: {self.audio_names[index]}")
            spkembs = get_features_or_waveform(
                os.path.join(self.audio_root, self.spk_embeds[index])
            )
            spkembs = torch.from_numpy(spkembs).float()
        # elif os.path.exists(str(os.path.join(self.audio_root, self.audio_names[index])).replace('.wav', '.npy')):
        #     spkembs = get_features_or_waveform(
        #         str(os.path.join(self.audio_root, self.audio_names[index])).replace('.wav', '.npy')
        #     )
        #     spkembs = torch.from_numpy(spkembs).float()
        elif os.path.exists(str(os.path.join('/root/TTS/v1.2/data/ar_emb/', self.audio_names[index].split('/')[-1])).replace('.wav', '.npy')):
            # print(f"From path: {str(os.path.join('/root/TTS/v1.2/data/ar_emb/', self.audio_names[index].split('/')[-1])).replace('.wav', '.npy')} for audio: {self.audio_names[index]}")
            spkembs = get_features_or_waveform(
                str(os.path.join(self.audio_root, self.audio_names[index])).replace('.wav', '.npy')
            )
            spkembs = torch.from_numpy(spkembs).float()
            ...
            spkembs = create_speaker_embedding(wav)
            # spk_emb_name = str(os.path.join(self.audio_root, self.audio_names[index])).replace('.wav', '.npy')
            spk_emb_name = str(os.path.join('/root/TTS/v1.2/data/ar_emb/', self.audio_names[index].split('/')[-1])).replace('.wav', '.npy')
            np.save(spk_emb_name, spkembs)
            spkembs = torch.from_numpy(spkembs).float()
