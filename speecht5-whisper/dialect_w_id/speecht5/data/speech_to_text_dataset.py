    ...
    with open(manifest_path) as f:
        ...
        for ind, line in enumerate(f):
            ...
            if min_keep is not None and sz < min_keep:
                n_short += 1
                # print(f"skipping short {items[0]}")
                ...
                inds.append(ind)
                sizes.append(sz)
                dialects.append(items[2] if len(items) > 2 else "Palestine")
    tot = ind + 1
    logger.info(
        (
            ...
            f"number of dialects={len(set(dialects))}"
        )
    )
    return root, names, inds, tot, sizes, dialects
...
class SpeechToTextDataset(FairseqDataset):
    def __init__(
        ...
        self.audio_root, self.audio_names, inds, tot, self.wav_sizes, self.dialects = load_audio(
    ...
    def get_label(self, index, label_idx):
        ...
        if self.tokenizer is not None:
            ...
            label = f"{self.dialects[index]} "  + self.tokenizer.encode(label)
            # breakpoint()
