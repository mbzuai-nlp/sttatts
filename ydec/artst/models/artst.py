...
class ArTSTTransformerModel(FairseqEncoderDecoderModel):
    ...
    def __init__(
            self, 
            args,
            encoder, tts_decoder, asr_decoder,
        ...
        super().__init__(encoder, tts_decoder)

        self.encoder = encoder
        # self.decoder = decoder

        # Hawau : adding separate decoders for ASR and TTS
    ...
    def build_model(cls, args, task):
        ...
        # Hawau : adding separate decoders for ASR and TTS
        asr_decoder = cls.build_decoder(args)
        tts_decoder = cls.build_decoder(args)
    ...
    def forward(self, source=None, src_tokens=None, src_lengths=None, prev_output_tokens=None, tgt_lengths=None, spkembs=None, target_list=None, task_name=None, padding_mask=None, only_hubert=False, only_ctc=False, feature_only=False, tgt_enc_layer=None, mask=True):
        ...
        # Decoder
        if task_name is not None and task_name == "t2s":
            # logger.info("t2s using tts_decoder")
            decoder_output, extra = self.tts_decoder(prev_output_tokens, tgt_mask, encoder_output, 
        ...
        elif task_name is not None and task_name == "s2t":
            # logger.info("s2t using asr_decoder")
            decoder_output, extra = self.asr_decoder(prev_output_tokens, tgt_mask, encoder_output, 
                                             full_context_alignment=getattr(self.args, "decoder_full_context_alignment", False), 
                                             alignment_layer=(-1 if target_list is None and output_type == 'speech' else None))
    ...
    def load_state_dict(
        ...
        # modify to allow loading checkpoint with different embedding size
        model_dict_size = self.speech_decoder_prenet.spkembs_layer[0].in_features
        ckpt_dict_size = state_dict["speech_decoder_prenet.spkembs_layer.0.weight"].size(1)
        if model_dict_size != ckpt_dict_size:
            logger.warn(f"not equal speaker embedding between model and checkpoint: {model_dict_size} vs {ckpt_dict_size}")
            logger.info(f"reset model speaker embedding with size of {model_dict_size}")
            removed_keys = [
                key for key in state_dict.keys() if (
                    key.startswith("speech_decoder_prenet.spkembs_layer")
                )
            ]
            for key in removed_keys:
                state_dict.pop(key, None)
...
# Hawau: adding separate decoders for ASR and TTS; changed self.decoder to self.asr_decoder
    def forward_decoder(self, tokens, encoder_out, incremental_state):
        ...
        # Decoder
        decoder_output, extra = self.asr_decoder(
            prev_output_tokens,
            tgt_mask,
    ...
    def generate_speech(self, source=None, src_tokens=None, spkembs=None, **kwargs):
        ...
        if isinstance(self.tts_decoder, FairseqIncrementalDecoder):
            incremental_states = {}
        else:
        ...
        while True:
            ...
            # Hawau : adding separate decoders for ASR and TTS
            z, extra = self.tts_decoder(decoder_in[:,-1:], None, encoder_out, incremental_states, alignment_layer=-1)
