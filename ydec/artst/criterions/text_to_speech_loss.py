...
class TexttoSpeechLoss(FairseqCriterion):
    ...
    def compute_loss(self, model, net_output, sample):
        ...
        # print(f"Input lengths: ilens{ilens}, olens{olens}")
        ...
        if self.use_guided_attn_loss:
            ...
            if hasattr(model, 'encoder_reduction_factor') and model.encoder_reduction_factor > 1:
                ...
                ilens_in = ilens.new([torch.div(ilen, model.reduction_factor, rounding_mode='floor') for ilen in ilens])
                ilens = ilens.new([ilen - ilen % model.reduction_factor for ilen in ilens])
            else:
                ilens_in = ilens
            ...
            # print(f"lengths after modification: ilens{ilens_in}, olens{olens_in}")
            ...
            if "encoder-decoder" in self.modules_applied_guided_attn:
                ...
                if att_ws.dim() == 4:
                    try:
                        ...
                        print(f"lengths after modification: ilens{ilens_in}, \n olens{olens_in}, \n attn_ws size:  {att_ws.size()} , attn")
